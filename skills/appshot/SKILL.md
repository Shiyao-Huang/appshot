...
Training is something **you (the agent) drive by calling tools**. The optional
`scripts/train_local_app_rules.py` helper only orchestrates capture, replay, evaluation,
and rule selection; its output must stay `ruleOutputKind: upsertable-json-rule`, with
JSON rule artifacts that can be fed back through `rules upsert` / `rules patch`.
AppShot's job is to extract structured window content through the **Accessibility (AX)
API**. OCR is a *teacher* used only to measure what is truly on screen; it is never emitted
as rule output, because the consuming agent is already multimodal and reads the screenshot
...
- `codexComputerUseStatus.hostBridge.codexHostIntegration`
- `codexComputerUseStatus.hostBridge.codexHostIntegration.privateCodexWebviewHostAttached`
...
- `codexBrowserDOMIntegration.browserRuntimeBridge.nativeCodexDesktopAvailable`
- `codexBrowserDOMIntegration.browserRuntimeBridge.codexHostBridgeAvailable`
- `codexBrowserDOMIntegration.browserRuntimeBridge.extensionHelperAvailable`
- `codexBrowserDOMIntegration.browserRuntimeBridge.electronHostBridgeAvailable`
- `codexBrowserDOMIntegration.browserRuntimeBridge.hostAPI`
- `codexBrowserDOMIntegration.browserRuntimeBridge.hostChannel`
- `codexBrowserDOMIntegration.browserRuntimeBridge.hostOwner`
- `codexBrowserDOMIntegration.browserRuntimeBridge.hostTransport`
