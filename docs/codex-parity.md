...
| Native App build | Extracted AppShot must ship as a real macOS app, not only a CLI/MCP surface | `AppShot.xcodeproj` scheme `AppShot`, bundle id `com.qppshot.AppShot`, version `0.1.5` | `scripts/verify_codex_parity.sh` Xcode build, bundle plist, executable, and codesign checks |
...
| Version/package alignment | Extracted app/plugin must match release identity | plugin, MCP package, MCP server, installer, release script at `0.1.5` | `scripts/verify_codex_parity.sh` version alignment |

## Evidence-Tracked But Not Implemented

| Area | Codex Evidence | Why Not Claimed Complete | Next Concrete Step |
| --- | --- | --- | --- |
| Design editor/tweaks DOM IPC | `browser-sidebar-runtime-open-design-editor`, `design-scrub`, `isTweaksEditorOpen` | AppShot now exposes `isTweaksEditorOpen` as adapter state, but does not target DOM elements or invoke Codex browser host/preload IPC | Keep evidence checked; do not present AppShot as implementing DOM design editor behavior |
