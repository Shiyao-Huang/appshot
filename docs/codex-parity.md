...
| Native App build | Extracted AppShot must ship as a real macOS app, not only a CLI/MCP surface | `AppShot.xcodeproj` scheme `AppShot`, bundle id `com.qppshot.AppShot`, version `0.1.11` | `scripts/verify_codex_parity.sh` Xcode build, bundle plist, executable, and codesign checks |
...
| Target AX window matching diagnostics | Codex native-module evidence distinguishes `frontmostWindow`, `currentWindow`, `CGWindow`, and AX-derived context; mismatches must be visible instead of silently returning the app menu tree | `accessibility.targetWindowMatch` reports candidate count, score, focused/main results, top candidates, and recovery steps; `rootSource` uses `targetWindowUnmatchedApplication` or `targetWindowUnmatchedFocusedWindow` when a requested CGWindow cannot be matched to an AXWindow | `scripts/verify_codex_parity.sh` schema checks plus manual VS Code `windowID` capture |
...
| Version/package alignment | Extracted app/plugin must match release identity | plugin, MCP package, MCP server, installer, release script at `0.1.11` | `scripts/verify_codex_parity.sh` version alignment |
...
- Target AX window matching loses `accessibility.targetWindowMatch`, candidate scoring diagnostics, recovery steps, or explicit `targetWindowUnmatchedApplication` / `targetWindowUnmatchedFocusedWindow` root-source values for requested CGWindow misses.
