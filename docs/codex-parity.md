...
Three sentences: The strongest local evidence is the extracted Codex Mac app bundle under `../codex-522/mac-app`, especially runtime event lists, app-session snippets, and the architecture note. AppShot currently implements the native macOS app/window/accessibility/screenshot side and exposes it through CLI, MCP, skill, plugin, release packaging, a Codex browser-comment payload adapter, the `browser-annotation-screenshots-mode` policy surface, a Codex browser runtime-state adapter, a Codex 522 browser runtime protocol adapter, and a Safari/Chrome DOM integration that emits Codex-shaped comment/design/image/screenshot event candidates. Continuous browser-preload event listening is still evidence-tracked because it belongs to Codex's embedded browser runtime.
...
| Native App build | Extracted AppShot must ship as a real macOS app, not only a CLI/MCP surface | `AppShot.xcodeproj` scheme `AppShot`, bundle id `com.qppshot.AppShot`, version `0.1.7` | `scripts/verify_codex_parity.sh` Xcode build, bundle plist, executable, and codesign checks |
...
| Version/package alignment | Extracted app/plugin must match release identity | plugin, MCP package, MCP server, installer, release script at `0.1.7` | `scripts/verify_codex_parity.sh` version alignment |

## Evidence-Tracked But Not Implemented

| Area | Codex Evidence | Why Not Claimed Complete | Next Concrete Step |
| --- | --- | --- | --- |
| Continuous browser preload event stream | `sendMessageToHost`, `subscribeToHostMessages`, `browser-sidebar-runtime-message`, `browser-sidebar-runtime-open-design-editor`, `design-scrub`, `browser-sidebar-runtime-image-drag-started`, `browser-sidebar-runtime-image-drag-ended` | AppShot now has a runtime protocol adapter plus read-only browser DOM integration and Codex-shaped event candidates, but does not install a persistent preload listener or receive real drag/scrub events as they happen | Add only with an embedded browser extension/preload helper |
