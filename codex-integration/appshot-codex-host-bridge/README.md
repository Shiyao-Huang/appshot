...
Run `scripts/analyze_codex_electron_host_injection.mjs` from a source checkout to verify the packaged Codex insertion points without mutating Codex.app. Installed releases also carry this analyzer, `scripts/patch_codex_electron_host_for_appshot.mjs`, and `scripts/materialize_codex_host_patch_for_appshot.mjs` inside the Codex integration directory. The analyzer and patcher discover hashed `main-*.js` bundles from Codex IPC anchors instead of a fixed filename. The patcher's default dry-run mode reports the exact main-bundle and `comment-preload.js` anchors it would patch; pass `--asar-root /path/to/unpacked/app.asar` for a real unpacked Codex asar, or `--codex-app /Applications/Codex.app` to inspect whether the app exposes an unpacked asar. If the app only contains a packed `Contents/Resources/app.asar`, run the materializer, for example `node ~/.local/share/appshot/codex-integration/appshot-codex-host-bridge/scripts/materialize_codex_host_patch_for_appshot.mjs --codex-app /Applications/Codex.app --output-dir /tmp/codex-appshot-host --copy-app-bundle --pretty`; it extracts `app.asar` plus `app.asar.unpacked`, patches a copy, syntax-checks the patched JS, and leaves the original Codex.app untouched.
