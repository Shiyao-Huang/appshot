...
codex_host_patcher = (root / "scripts/patch_codex_electron_host_for_appshot.mjs").read_text()
codex_host_materializer = (root / "scripts/materialize_codex_host_patch_for_appshot.mjs").read_text()
...
for name, text, needles in [
    ...
    ("Codex host integration adapter", codex_host_adapter + codex_host_readme + codex_host_verifier + codex_host_analyzer + codex_host_patcher + codex_host_materializer + core + installer + release + parity + skill, ["codex-host-adapter.cjs", "installAppShotCodexHostBridge", "wrapCodexRuntimeMessageHandler", "analyze_codex_electron_host_injection.mjs", "patch_codex_electron_host_for_appshot.mjs", "materialize_codex_host_patch_for_appshot.mjs", "--asar-root", "--codex-app", "--copy-app-bundle", "packed app.asar", "app.asar.extracted", "sourceMainBundle", "outputAsarRoot", "syntaxChecks", "codex_desktop:browser-sidebar-runtime-message", "codex_desktop:message-for-view", "hostInboundChannel", "hostOutboundChannel", "ipcRenderer.invoke/ipcMain.handle", "webContents.send/ipcRenderer.on", "sendMessageToHost", "subscribeToHostMessages", "codex-electron-host", "codex-electron-ipc+appshot-electron-ipc", "host-managed-browser-state", "codexHostIntegration", "privateCodexWebviewHostAttached", "scripts/verify_codex_host_integration.mjs"]),
...
if governance.get("ruleOutputKind") != "upsertable-json-rule":
    raise SystemExit(f"rule governance output is not JSON-rule based: {governance}")
...
if training.get("ruleOutputKind") != "upsertable-json-rule":
    raise SystemExit(f"rule trainer output is not upsertable JSON rule artifacts: {training}")
if training.get("ruleArtifactFormat") != "json":
    raise SystemExit(f"rule trainer artifact format drifted: {training}")
if training.get("renderedRuleCount", 0) < 1:
    raise SystemExit(f"rule trainer rendered no rule JSON artifacts: {training}")
