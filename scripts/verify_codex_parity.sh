...
for name, text, needles in [
    ...
    ("Codex host integration adapter", codex_host_adapter + codex_host_readme + codex_host_verifier + codex_host_analyzer + core + installer + release + parity + skill, ["codex-host-adapter.cjs", "installAppShotCodexHostBridge", "wrapCodexRuntimeMessageHandler", "analyze_codex_electron_host_injection.mjs", "patch_codex_electron_host_for_appshot.mjs", "codex_desktop:browser-sidebar-runtime-message", "codex_desktop:message-for-view", "hostInboundChannel", "hostOutboundChannel", "ipcRenderer.invoke/ipcMain.handle", "webContents.send/ipcRenderer.on", "sendMessageToHost", "subscribeToHostMessages", "codex-electron-host", "codex-electron-ipc+appshot-electron-ipc", "host-managed-browser-state", "codexHostIntegration", "privateCodexWebviewHostAttached", "scripts/verify_codex_host_integration.mjs"]),
