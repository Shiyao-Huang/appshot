...
- Codex host integration adapter：`codex-integration/appshot-codex-host-bridge` 会作为 Codex-side host adapter 打包和安装。它导出 `installAppShotCodexHostBridge`，复用 Electron helper，报告 `hostOwner: codex-electron-host`，并按 Codex 522 的真实双通道 IPC 工作：`codex_desktop:browser-sidebar-runtime-message` 用于 preload-to-host `ipcRenderer.invoke` / `ipcMain.handle`，`codex_desktop:message-for-view` 用于 host-to-preload `webContents.send` / `ipcRenderer.on`。`scripts/analyze_codex_electron_host_injection.mjs` 可以在不修改 Codex.app 的情况下验证这些打包产物接入点；release/install 也会把 analyzer、`patch_codex_electron_host_for_appshot.mjs` 和 `materialize_codex_host_patch_for_appshot.mjs` 放进已安装的 Codex integration 目录，所以显式 Codex host patch 实验不需要源码 checkout。materializer 可以把 packed `app.asar` 和 `app.asar.unpacked` 解进 patched copy，同时不修改原始 Codex.app；在真实 Codex Electron host 接入之前，standalone CLI/MCP 会保持 `privateCodexWebviewHostAttached: false`。
